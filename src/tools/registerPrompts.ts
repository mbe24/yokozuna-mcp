...
1. SHAPE: sumo_facets (where do matching logs come from; which levels/statuses/paths dominate) and/or sumo_trend (WHEN did it start/spike). Use detail:"summary" on sumo_run_search for exact per-level counts + histogram.
...
- WHICH ENV: add the deployment hostname as a keyword (e.g. "www.example.com") — CAVEAT: hostname keywords match only request logs (the hostname lives in log.request_url); startup/worker lines and most errors/exceptions carry NO hostname and get excluded. Hunt errors by _sourcecategory, never by hostname keyword.
- SEVERITY (reliable form): ... | json field=_raw "log.levelname" as levelname nodrop | where levelname in ("ERROR","WARNING")
  Do NOT filter on _loglevel (often empty on warnings; says "WARN" where levelname says "WARNING") and stream:"stderr" is NOT an error signal.
- COUNT by level: ... | json field=_raw "log.levelname" as levelname nodrop | count by levelname
